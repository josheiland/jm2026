// Single source of truth for the weekend.
//
// Everything here is sourced from the Zola site (zola.com/wedding/eilands2026) or
// from confirmed vendor notes. Times are written as ISO strings with an explicit
// -04:00 offset: Charlottesville is on EDT the whole weekend (US DST 2026 runs
// Mar 8 – Nov 1), so the offset is constant and no timezone library is needed.
// Pinning the offset means the countdown and "happening now" logic are correct
// for a guest sitting in California, London, or Kigali.

export const ET = '-04:00'

export type EventKind = 'party' | 'ceremony' | 'shuttle' | 'brunch'

export interface WeddingEvent {
  slug: string
  name: string
  kind: EventKind
  /** Short label for the timeline rail. */
  time: string
  start: string
  end: string
  day: 'sat' | 'sun' | 'mon'
  venue: string
  address: string
  /** Google Maps needs the full "name, address" string to land on the right pin. */
  mapQuery: string
  attire?: string
  attireNote?: string
  description?: string
  /** Rendered as a callout — the things people actually get wrong. */
  heads_up?: string
}

export const WEDDING_DATE = `2026-09-06T17:00:00${ET}`

export const DAYS = [
  { key: 'sat', label: 'Saturday', date: 'September 5', long: 'Saturday, September 5, 2026' },
  { key: 'sun', label: 'Sunday', date: 'September 6', long: 'Sunday, September 6, 2026' },
  { key: 'mon', label: 'Monday', date: 'September 7', long: 'Monday, September 7, 2026' },
] as const

export const EVENTS: WeddingEvent[] = [
  {
    slug: 'welcome-party',
    name: 'Welcome Party',
    kind: 'party',
    time: '4:00 – 8:00 pm',
    start: `2026-09-05T16:00:00${ET}`,
    end: `2026-09-05T20:00:00${ET}`,
    day: 'sat',
    venue: 'Starr Hill Brewery Downtown at the Dairy Market',
    address: '946 Grady Ave, Suite 101, Charlottesville, VA 22903',
    mapQuery: 'Starr Hill Brewery Downtown, 946 Grady Ave, Charlottesville, VA 22903',
    attire: 'Elevated casual',
    attireNote: 'Relaxed, polished, and festive.',
    description:
      'Drop by whenever! We will have appetizers and drinks starting from the beginning of the event and a full buffet dinner starting around 6pm!',
    heads_up: 'No shuttle for this one — the Dairy Market is a walk or a short ride from the West Main hotels.',
  },
  {
    slug: 'shuttle-out',
    name: 'Shuttle to the Vineyard',
    kind: 'shuttle',
    time: '4:15 – 4:45 pm',
    start: `2026-09-06T16:15:00${ET}`,
    end: `2026-09-06T16:45:00${ET}`,
    day: 'sun',
    venue: 'In front of The Draftsman',
    address: '1106 West Main Street, Charlottesville, VA 22903',
    mapQuery: 'The Draftsman Autograph Collection, 1106 West Main St, Charlottesville, VA 22903',
    description:
      'Buses load in front of The Draftsman and run out to Hazy Mountain. Staying at the Hampton Inn or the Courtyard? They are both a few minutes’ walk down West Main — come to The Draftsman to board.',
    heads_up:
      'This is the one time all weekend worth setting an alarm for. It is a 35-minute drive to the vineyard and the last bus pulls out at 4:45pm.',
  },
  {
    slug: 'wedding',
    name: 'The Wedding',
    kind: 'ceremony',
    time: '5:00 – 11:00 pm',
    start: `2026-09-06T17:00:00${ET}`,
    end: `2026-09-06T23:00:00${ET}`,
    day: 'sun',
    venue: 'Hazy Mountain Vineyards & Brewery',
    address: '240 Hazy Mountain Ln, Afton, VA 22920',
    mapQuery: 'Hazy Mountain Vineyards & Brewery, 240 Hazy Mountain Ln, Afton, VA 22920',
    attire: 'Summer formal',
    attireNote:
      'Most folks will be in long dresses and dark suits — but wear whatever makes your heart happy. It can’t be that deep, we’re just excited to celebrate with you.',
    heads_up:
      'The ceremony is outdoors on a working vineyard. Grass plus stiletto heels is a losing combination — block heels or a pair of heel stoppers will save you.',
  },
  {
    slug: 'shuttle-early',
    name: 'Early Shuttle Back',
    kind: 'shuttle',
    time: '9:30 – 10:00 pm',
    start: `2026-09-06T21:30:00${ET}`,
    end: `2026-09-06T22:00:00${ET}`,
    day: 'sun',
    venue: 'Hazy Mountain → The Draftsman',
    address: '240 Hazy Mountain Ln, Afton, VA 22920',
    mapQuery: 'Hazy Mountain Vineyards & Brewery, 240 Hazy Mountain Ln, Afton, VA 22920',
    description: 'The civilized option. Straight back to The Draftsman on West Main.',
  },
  {
    slug: 'shuttle-late',
    name: 'Late Shuttle Back',
    kind: 'shuttle',
    time: '11:00 – 11:30 pm',
    start: `2026-09-06T23:00:00${ET}`,
    end: `2026-09-06T23:30:00${ET}`,
    day: 'sun',
    venue: 'Hazy Mountain → The Draftsman',
    address: '240 Hazy Mountain Ln, Afton, VA 22920',
    mapQuery: 'Hazy Mountain Vineyards & Brewery, 240 Hazy Mountain Ln, Afton, VA 22920',
    description: 'Leaves when the reception ends, with an optional bar stop on the way home.',
  },
  {
    slug: 'after-party',
    name: 'After Party',
    kind: 'party',
    time: '11:30 pm – 1:00 am',
    start: `2026-09-06T23:30:00${ET}`,
    end: `2026-09-07T01:00:00${ET}`,
    day: 'sun',
    venue: 'The Corner',
    address: 'The Corner, Charlottesville, VA 22903',
    mapQuery: 'The Corner, Charlottesville, VA 22903',
    description:
      'Back on the Corner, exactly where this all started. The late shuttle drops close by — we’ll share the specific bar in the WhatsApp group once we lock it.',
  },
  {
    slug: 'bagels',
    name: 'Bagels by the Lawn',
    kind: 'brunch',
    time: '10:00 am – 12:00 pm',
    start: `2026-09-07T10:00:00${ET}`,
    end: `2026-09-07T12:00:00${ET}`,
    day: 'mon',
    venue: 'The Lawn',
    address: '400 Emmet Street South, Charlottesville, VA 22903',
    mapQuery: 'The Lawn, University of Virginia, Charlottesville, VA 22903',
    attire: 'Groutfits, sweats, jammies',
    attireNote: 'All the cozies welcome.',
    description: 'Roll out of bed and right onto the Lawn if you’d like!',
    heads_up: 'Bodo’s. Obviously. Josh will be eating a pastrami, egg and muenster on an everything bagel.',
  },
]

export const eventBySlug = (slug: string) => EVENTS.find((e) => e.slug === slug)

export const eventsForDay = (day: 'sat' | 'sun' | 'mon') => EVENTS.filter((e) => e.day === day)

/** Sunset at the vineyard, for anyone planning golden-hour photos. */
export const SUNSET = { sat: '7:39 pm', sun: '7:37 pm', mon: '7:35 pm' }

export const mapsUrl = (q: string) =>
  `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q)}`
